...
class EVRPTW:
    """
    Implementation of E-VRP-TW-V2G as a generalized disjunctive model
    ...
    def __init__(self, problem_type: str):
        ...
        :param problem_type: Objective options include: {Schneider} OR {OpEx, CapEx, Cycle, EA, DCM, Delivery}
         ...
         Transformatiom methods include: {Hull} OR {BigM}
        """
    ...
    def build_model(self):
        ...
        self.m.S_ = Set(dimen=1, doc='All charging station nodes extended') #TODO: does it contain D0?
        ...
        self.m.Smap = Param(self.m.S, domain=Any, doc='Mapping from original charging station to all its duplicates') #TODO: why?
        ...
        def ArcOn(d, i, j):
            if i != j:
                ...
                d.constraint_xgamma = Constraint(expr=m.xgamma[i,j] == 1, doc='Arc must be activated')

                if i in m.M:
                    '''Constraints that are unique to customer nodes'''

                    d.constraint_time_customer = Constraint(expr=inequality(None, m.xw[i] + (m.tS[i] + m.d[i, j] / m.v + m.tQ[i] * m.q[i]), m.xw[j]), doc='Service time for each vehicle doing delivery at each customer node')
                    d.constraint_energy_customer = Constraint(expr=inequality(None, m.xa[j], m.xa[i] - (m.r * m.d[i, j])), doc='Energy transition for each EV while at customer node i and traveling across edge (i, j)')
                    # d.constraint_energy_customer = Constraint(expr=m.xa[j] == m.xa[i] - (m.r * m.d[i, j]), doc='Energy transition for each EV while at customer node i and traveling across edge (i, j)')
                    d.constraint_payload = Constraint(expr=inequality(None, m.xq[j], (m.xq[i] - m.q[i])), doc='Vehicles must unload payload for full customer demand when visiting a customer')
                ...
                if i in m.start_node: #TODO: merge with customer block?
                    '''Constraints that are unique to the start node'''
                    d.constraint_time_start_depot = Constraint(expr=inequality(None, m.xw[i] + (m.tS[i] + m.d[i, j] / m.v), m.xw[j]), doc='Service time for each vehicle at start depot')
                    d.constraint_energy_customer = Constraint(expr=inequality(None, m.xa[j], m.xa[i] - (m.r * m.d[i, j])), doc='Energy transition for each EV while at customer node i and traveling across edge (i, j)')
                    # d.constraint_energy_customer = Constraint(expr=m.xa[j] == m.xa[i] - (m.r * m.d[i, j]), doc='Energy transition for each EV while at customer node i and traveling across edge (i, j)')
                    d.contraint_payload_start = Constraint(expr=m.xq[i] == m.QMAX, doc='Full payload limits at the start node')
        ...
        def EVChargeDischarge(d, i, j, t): #TODO: revert to i,t indexes (did this to include noxkappabounds constraints)
            if i != j:
                m = d.model()
                d.constraint_xkappa = Constraint(expr=m.xkappa[i,t] == 1, doc='EV is charging or discharging')
        ...
        def EVIdle(d, i, j, t):
            if i != j:
                m = d.model()
                d.constraint_xkappa = Constraint(expr=m.xkappa[i,t] == 0, doc='EV is idle')
        ...
        if 'nosymmetry' not in problem_type:
            def constraint_station_symmetry(m, i): #TODO: Show constraint
        ...
        # VARIOUS GLOBAL CONSTRAINTS #TODO: some of these may be moved inside disjuncts with attention to input sets
        ...
        def constraint_xgamma_xkappa(m, i, t):
            ...
            return m.xkappa[i, t] <= sum(m.xgamma[i, j] for j in m.V1_ if i != j) #TODO: review
        ...
        def constraint_terminal_node_time(m, i):  # TODO: Could remove if m.tB[i] = m.t_T - m.tS[i]
            """Arrival time must be within time window for each node""" #TODO: why? depot has no service time and t_T = tB?
