...
def plot_evrptwv2g(m: 'obj'):
    ...
    for i, n in enumerate('SDM'):
        ...
        axs_graph.scatter(m.data[n]['d_x'], m.data[n]['d_y'],
                     c=[node_colors_rgba_tuple[n]] * len(m.data[n]), s=100)
    ...
    for n in tA.keys():
        if 'D' in n:
            axs_tw.broken_barh([(tA[n], tB[n] - tA[n])], (i, tw_height), facecolors=node_colors_rgba_tuple[n[0]])
            tw_labels[n] = i
            i += 1
    ...
    for n in tA.keys():
        if n in s_color.keys():
            ...
            axs_tw.broken_barh([(tA[n], tB[n] - tA[n])], (i, tw_height), facecolors=node_colors_rgba_tuple['S'])
            tw_labels[n] = i
            i += 1
    ...
    for ti, t in enumerate(traces):
        ...
        ax_q.grid(True, which='both', axis='x', alpha=5, linestyle=':')
        ...
        ax_soe.grid(True, which='both', axis='x', alpha=5, linestyle=':')
    ...
    axs_p.grid(True, which='both', axis='x', alpha=5, linestyle=':')
    ...
    axs_ce.grid(True, which='both', axis='x', alpha=5, linestyle=':')
    ...
    for si, s in enumerate(unique_stations):
        ...
        ax_g.grid(True, which='both', axis='x', alpha=5, linestyle=':')
