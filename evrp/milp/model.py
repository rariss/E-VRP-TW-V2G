...
class MILP:
    ...
    def define_constraints(self):
        ...
        def constraint_arcs(m, i):
            """Incoming arcs = Outgoing arcs"""
            return sum([self.m.xgamma[i, j1] for j1 in self.m.V1 - i]) - sum(
                [self.m.xgamma[j0, i] for j0 in self.m.V0 - i]) == 0

        # Create constraint
        # self.m.constraint_arcs = Constraint(self.m.V, rule=constraint_arcs)

        def constraint_visits(m, i):
            # if i in ['d-1']:
            #     return sum([self.m.xgamma[j, i] for j in self.m.V-i]) == 1
            # if i in ['d0']:
            #     return sum([self.m.xgamma[i, j] for j in self.m.V]) == 1
            # else:
            return sum([self.m.xgamma[i, j] for j in self.m.V1 - i]) == 1

        # Create constraint
        # self.m.constraint_visits = Constraint(self.m.V0, rule=constraint_visits)
