...
class EVRPTWV2G:
    ...
    def build_model(self):
        ...
        self.m.V0_ = Set(initialize=self.m.V01_ - self.m.end_node,
                         doc='All nodes extended except ending depot node')
        self.m.V1_ = Set(initialize=self.m.V01_ - self.m.start_node,
                         doc='All nodes extended except starting depot node')
        ...
        self.m.tS = Param(self.m.V01_, mutable=True,
                          doc='Fixed service time for a vehicle at node i')
        self.m.tQ = Param(self.m.V01_, mutable=True,
                          doc='Payload service time for a vehicle at node i')
        ...
        self.m.ce = Param(self.m.S, self.m.T, mutable=True,
                          doc='Amortized operating cost for energy charges')
        self.m.cq = Param(self.m.M, mutable=True, doc='Amortized revenue for deliveries')
        self.m.Smap = Param(self.m.S, domain=Any,
                            doc='Mapping from original charging station to all its duplicates')
        ...
        self.m.xw = Var(self.m.V01_, within=NonNegativeReals,
                        bounds=(0, self.m.MT))  # Arrival time for each vehicle at each node
        self.m.xq = Var(self.m.V01_, within=NonNegativeReals,
                        bounds=(0, self.m.MQ))  # Payload of each vehicle before visiting each node
        self.m.xa = Var(self.m.V01_, within=NonNegativeReals, initialize=self.m.EMAX,
                        bounds=(0, self.m.ME))  # Energy of each EV arriving at each node
        self.m.xd = Var(self.m.S, within=NonNegativeReals,
                        initialize=0)  # Each station’s net peak electric demand
        self.m.xp = Var(self.m.S_, self.m.T, within=Reals, bounds=(
            -self.m.PMAX, self.m.PMAX))  # AC Discharge or charge rate of each EV
        if 'splitxp' in self.problem_types:
            self.m.xc = Var(self.m.S_, self.m.T, within=NonNegativeReals, initialize=0,
                            bounds=(0, self.m.PMAX))  # AC Charge rate of each EV
            self.m.xg = Var(self.m.S_, self.m.T, within=NonNegativeReals, initialize=0,
                            bounds=(0, self.m.PMAX))  # AC Discharge rate of each EV
        ...
        def disjunct_v2g_state(m, i, t):
            return [m.xkappa[i, t], m.xkappa_off[i, t]]
        ...
        def disjunct_arc_on(d, i, j):
            if i != j:
                ...
                # For all nodes i
                # Turn on xgamma[i, j] variable
                # d.constraint_xgamma_on = Constraint(expr=m.xgamma[i, j] == 1)

                # Arrival time must be within time window for each node
                d.constraint_node_time_window = Constraint(
                    expr=inequality(m.tA[i], m.xw[i], m.tB[i]))
                ...
                if i in m.start_node:
                    ...
                    d.constraint_time_start_depot = Constraint(
                    ...
                    d.constraint_energy_customer = Constraint(
                ...
                if i in m.M:
                    ...
                    d.constraint_time_customer = Constraint(
                    ...
                    d.constraint_payload = Constraint(expr=m.xq[j] <= m.xq[i] - m.q[i])

                # For charging stations i in S_
                if i in m.S_:
                    # Time Constraints
                    # Service time for each EV doing V2G/G2V at each charging station
                    d.constraint_time_station = Constraint(
        ...
        self.m.xgamma = Disjunct(self.m.E, rule=disjunct_arc_on)

        def disjunct_arc_off(d, i, j):
            m = d.model()
            return Disjunct.Skip

        self.m.xgamma_off = Disjunct(self.m.E, rule=disjunct_arc_off)

        def disjunct_arc_state(m, i, j):
            return [m.xgamma[i, j], m.xgamma_off[i, j]]
        ...
        def constraint_logical(m, i, t):
            return m.xkappa[i, t].binary_indicator_var - sum(
                m.xgamma[i, j].binary_indicator_var for j in m.V1_ if i != j) <= 0

        self.m.constraint_logical = Constraint(self.m.S_, self.m.T, rule=constraint_logical)

        def constraint_energy_station_limit_lb(m, i,
                                               t):  # TODO: Combine duplicates to be within limits at each time
            """Charge limits for an EV at charging station i"""
            if 'splitxp' in self.problem_types:
        ...
        self.m.constraint_energy_station_limit_lb = Constraint(self.m.S_, self.m.T,
                                                               rule=constraint_energy_station_limit_lb)

        def constraint_energy_station_limit_ub(m, i,
                                               t):  # TODO: Combine duplicates to be within limits at each time
            """Charge limits for an EV at charging station i"""
            if 'splitxp' in self.problem_types:
        ...
        self.m.constraint_energy_station_limit_ub = Constraint(self.m.S_, self.m.T,
                                                               rule=constraint_energy_station_limit_ub)
        ...
        if 'nominvehicles' not in self.problem_types:
            ...
            self.m.constraint_min_vehicles = Constraint(self.m.start_node,
                                                        rule=constraint_min_vehicles)
        ...
        if 'maxvehicles' in self.problem_types:
            ...
            self.m.constraint_max_vehicles = Constraint(self.m.start_node,
                                                        rule=constraint_max_vehicles)
            ...
            def constraint_station_symmetry(m, i):
                ...
                if duplicate_number >= 1:  # check if it is a duplicate node
                    ...
                    return sum(m.xgamma[i, j].binary_indicator_var for j in m.V1_ if i != j) <= \
                           sum(m.xgamma[previous_duplicate, j].binary_indicator_var for j in m.V1_
                               if previous_duplicate != j)
                else:
                    return Constraint.Skip

            self.m.constraint_station_symmetry = Constraint(self.m.S_,
                                                            rule=constraint_station_symmetry)
        ...
        if 'stationaryevs' not in self.problem_types:
            def constraint_no_stationary_evs(m, i, j, s_):
                ...
                return m.xgamma[i, s_].binary_indicator_var + m.xgamma[
                    s_, j].binary_indicator_var <= 1

            self.m.constraint_no_stationary_evs = Constraint(self.m.start_node, self.m.end_node,
                                                             self.m.S_,
                                                             rule=constraint_no_stationary_evs)

        def constraint_terminal_node_time(m, i):  # TODO: Could remove if m.tB[i] = m.t_T - m.tS[i]
            """Arrival time must be within time window for each node"""
            return m.xw[i] + m.tS[i] <= m.t_T

        self.m.constraint_terminal_node_time = Constraint(self.m.end_node,
                                                          rule=constraint_terminal_node_time)
        ...
        if 'noxkappabounds' not in self.problem_types:
            def constraint_time_xkappa_lb(m, i, t):  # Initially missing
                ...
                return float(m.t_T - t) * m.xkappa[i, t].binary_indicator_var - m.MT * (
                        1 - m.xkappa[i, t].binary_indicator_var) <= m.t_T - m.xw[i]

            self.m.constraint_time_xkappa_lb = Constraint(self.m.S_, self.m.T,
                                                          rule=constraint_time_xkappa_lb)

            def constraint_time_xkappa_ub(m, i, j, t):
                """V2G decisions must be made before departure from the node"""
                if i != j:
                    return (m.tS[i] + m.d[i, j] / m.v) * m.xgamma[
                        i, j].binary_indicator_var + float(t + m.t_S) * m.xkappa[
                               i, t].binary_indicator_var - m.MT * (
                                   1 - m.xgamma[i, j].binary_indicator_var) <= m.xw[j]
                else:
                    return Constraint.Skip

            self.m.constraint_time_xkappa_ub = Constraint(self.m.S_, self.m.V1_, self.m.T,
                                                          rule=constraint_time_xkappa_ub)
        ...
        if 'start=end' in self.problem_types:
            ...
            self.m.constraint_energy_start_end_soe = Constraint(self.m.start_node, self.m.end_node,
                                                                rule=constraint_energy_start_end_soe)
            ...
            self.m.constraint_energy_start_end_soe = Constraint(self.m.start_node,
                                                                rule=constraint_energy_start_end_soe)
        ...
        def constraint_xgamma_xkappa(m, i, t):  # Initially missing
            ...
            return m.xkappa[i, t].binary_indicator_var <= sum(
                m.xgamma[i, j].binary_indicator_var for j in m.V1_ if i != j)  # TODO: review

        self.m.constraint_xgamma_xkappa = Constraint(self.m.S_, self.m.T,
                                                     rule=constraint_xgamma_xkappa)

        def constraint_energy_soe_station(m, i, t):  # Initially missing
            """Minimum and Maximum SOE limit for each EV"""
            if 'splitxp' in self.problem_types:
                return inequality(m.EMIN, m.xa[i] + m.t_S * sum(
                    m.eff * m.xc[i, b] - m.xg[i, b] / m.eff for b in m.T if b <= t), m.EMAX)
            else:
                return inequality(m.EMIN, m.xa[i] + m.t_S * sum(m.xp[i, b] for b in m.T if b <= t),
                                  m.EMAX)

        self.m.constraint_energy_soe_station = Constraint(self.m.S_, self.m.T,
                                                          rule=constraint_energy_soe_station)
    ...
    def obj(self, m):
        ...
        if 'schneider' in self.problem_types:
            ...
            if 'opex' in self.problem_types:
                obj_result += self.O_maintenance_operating_cost(m) + self.O_delivery_operating_cost(
                    m)
    ...
    def C_fleet_capital_cost(self, m):
        ...
        return m.cc * sum(
            sum(m.xgamma[i, j].binary_indicator_var for j in m.V1_ if j != i) for i in m.start_node)
    ...
    def R_energy_arbitrage_revenue(self, m):
        ...
        if 'splitxp' in self.problem_types:
            return -m.t_S * sum(
                sum(sum(m.ce[s, t] * (m.xc[i, t] - m.xg[i, t]) for t in m.T) for i in m.Smap[s]) for
                s in m.S)
        else:
            return -m.t_S * sum(
                sum(sum(m.ce[s, t] * m.xp[i, t] for t in m.T) for i in m.Smap[s]) for s in m.S)
    ...
    def total_distance(self, m):
        ...
        return sum(
            sum(m.d[i, j] * m.xgamma[i, j].binary_indicator_var for i in m.V0_ if i != j) for j in
            m.V1_)
    ...
    def cycle_cost(self, m):
        ...
        if 'splitxp' in self.problem_types:
            ...
            return m.cy * m.t_S * sum(
                sum(m.xkappa[i, t].binary_indicator_var for t in m.T) for i in m.S_)
        ...
        self.p = {
            None: {
                ...
                'd': {(k[0], k[1]): d for k, d in self.data['d'].stack().iteritems()
                      if k[0] != k[1] if k[0] != self.data['end_node'] if
                      k[1] != self.data['start_node']},
                # Parameter Sets  # self.data['d'].stack().to_dict()
    ...
    def import_instance(self, instance_filepath: str):
        ...
        for col in self.data['T'].columns.get_level_values(
                0).drop_duplicates():  # Produces energy price (ce) and demand (G) timeseries
            self.data[col] = self.data['T'][col].reindex(range(0, self.t_T, self.t_S),
                                                         method='ffill')

        # Create duplicates of charging stations
        log.info('Creating duplicates and extended graph')
        self.data['S_'] = self.data['S'].reindex(
            self.data['S'].index.repeat(self.data['S']['instances']))
        self.data['S_'].set_index(pd.Index(
            self.data['S_'].index + '_' + self.data['S_'].groupby(['node_id']).cumcount().astype(
                str)), inplace=True)
        ...
        self.s2s_ = {s: [s_ for s_ in self.data['S_'].index if s + '_' in s_] for s in
                     self.data['S'].index}  # S -> S_
        self.s_2s = {s_: s for s in self.data['S'].index for s_ in self.data['S_'].index if
                     s + '_' in s_}  # S_ -> S
    ...
    def make_instance(self, add_to_instance_name=''):
        ...
        self.instance.name = '{} {}{}'.format(self.instance_name, self.problem_type,
                                              add_to_instance_name)
        ...
        if 'gdpopt' not in self.problem_types:
            if 'hull' in self.problem_types:
                ...
                log.error(
                    'Must specify either "hull" or "bigm" in problem_type to define gdp instance.')
    ...
    def full_solve(self, instance_filepath: str):
        ...
        if 'gdpopt' in self.problem_types:
            self.results = self.opt.solve(self.instance, tee=True, strategy='LOA',
                                          time_limit=60 * 15)
        else:
    ...
    def solve(self):
        ...
        self.model_build_duration = (
                self.model_build_end_time - self.model_build_start_time).total_seconds()
        if 'gdpopt' not in self.problem_types:
            self.model_xfrm_duration = (
                    self.model_xfrm_end_time - self.model_build_end_time).total_seconds()
            self.model_solve_duration = (
                    self.model_solve_end_time - self.model_xfrm_end_time).total_seconds()
        else:
            self.model_solve_duration = (
                    self.model_solve_end_time - self.model_build_end_time).total_seconds()
    ...
    def warmstart_solve(self):
        ...
        self.results = self.opt.solve(self.instance, tee=True, warmstart=True,
    ...
    def set_xgamma(self, archived_instance_name: str):
        ...
        self.instance.xgamma.set_values(
            {k: round(v) for k, v in
             self.instance_archive[archived_instance_name].xgamma.extract_values().items()})
